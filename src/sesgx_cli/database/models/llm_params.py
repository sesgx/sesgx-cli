...
class LLMParams(Base):
    __tablename__ = "llm_params"
    ...
    def get_or_save(
        ...
        stmt = (
            select(LLMParams)
            .where(LLMParams.kmeans_n_clusters == kmeans_n_clusters)
            .where(LLMParams.umap_n_neighbors == umap_n_neighbors)
        )
